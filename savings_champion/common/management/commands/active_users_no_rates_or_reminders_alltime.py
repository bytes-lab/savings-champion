...
class Command(NoArgsCommand):
    ...
    def handle_noargs(self, **options):
        ...
        fwr.writerow(
            ['Username', 'Email', 'First Name', 'Last Name', 'Tel. no.', 'Source', 'Last Login', 'Date Joined'])

        users = User.objects.filter(is_active=True, is_superuser=False).order_by('date_joined').exclude(
            email__in=SAVCHAMP_EMAIL_FILTERS)
        ...
        if counter > 0:
            ...
            email = EmailMessage(
                ...
                'Please find attached a list of users who have joined and are not tracking any products.  ',
            ...
            email = EmailMessage('Portfolios and Trackers check w/e %s, 0 users founds ' % human_readable_format,
                                 'All users who have joined are tracking products',
                                 settings.DEFAULT_FROM_EMAIL,
                                 ALERTS_CSV_EMAIL, )
            email.send()